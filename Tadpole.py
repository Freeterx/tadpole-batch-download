...
'''
"""This module downloads all photos/videos from tadpoles to a local folder."""
...
class Client:
    ...
    def switch_windows(self):
        ...
        other = all_windows - current_window
        if other:
            other_window = other.pop()
    ...
    def do_login(self):
        ...
        self.browser.find_element_by_xpath("//div[@class='well']/div[2]").click()
        
        # Wait google auth process finish
        self.logger.info("Sleeping 30 seconds - wait google authorization to complete")
        self.sleep(minsleep=30, maxsleep=60)

    def iter_monthyear(self):
        '''Yields pairs of xpaths for each year/month tile on the
        right hand side of the user's home page.
        '''
        month_xpath_tmpl = '//*[@id="app"]/div[3]/div[1]/ul/li[%d]'
        month_index = 1
        while True:
            month_xpath = month_xpath_tmpl % (month_index)
            year_xpath = month_xpath_tmpl % (month_index)
